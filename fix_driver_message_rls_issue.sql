...
DO $$
BEGIN
  RAISE NOTICE 'Fixing Driver Message RLS Issue...';
END $$;
...
-- Step 2: Ensure all drivers have corresponding user accounts
DO $$
BEGIN
  RAISE NOTICE 'Step 2: Ensuring all drivers have corresponding user accounts...';
END $$;

-- Link any drivers that don't have user accounts
-- This creates user accounts for drivers that exist but don't have auth accounts
INSERT INTO public.users (id, role, name, email, created_at, updated_at)
SELECT 
  gen_random_uuid() as id,
  'Driver' as role,
  d.name,
  LOWER(REPLACE(d.name, ' ', '.')) || '@company.com' as email,
  NOW() as created_at,
  NOW() as updated_at
...
-- Step 3: Update drivers table to link with user accounts
DO $$
BEGIN
  RAISE NOTICE 'Step 3: Linking drivers to user accounts...';
END $$;

-- For drivers that don't have user_id set, try to link them by name
...
-- Step 4: Recreate load_messages RLS policies with better conditions
DO $$
BEGIN
  RAISE NOTICE 'Step 4: Recreating load_messages RLS policies...';
END $$;
...
FOR INSERT WITH CHECK (
  -- Allow if user is a driver AND (load is assigned to them OR they have general driver permissions)
  ...
  AND (
    ...
    OR
    -- Fallback: allow if user is a driver with valid driver record
    get_user_driver_id() IS NOT NULL
  )
);
...
-- Step 5: Create a temporary policy for testing (will be removed later)
CREATE POLICY "load_messages_driver_temp_debug" ON public.load_messages
FOR ALL USING (
  get_user_role() = 'Driver' 
);
...
DO $$
BEGIN
  RAISE NOTICE 'Step 5: Granting necessary permissions...';
END $$;
...
-- Step 6: Create a function to test driver message permissions
...
-- Step 7: Show current driver-user mappings for verification
DO $$
BEGIN
  RAISE NOTICE 'Step 6: Current driver-user mappings:';
END $$;
...
DO $$
BEGIN
  RAISE NOTICE '=== Driver Message RLS Fix Complete ===';
  RAISE NOTICE 'If issues persist, check:';
  RAISE NOTICE '1. Driver is properly authenticated in the app';
  RAISE NOTICE '2. Load is assigned to the correct driver_id';
